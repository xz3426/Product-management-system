...
const Cart = () => {
  ...
  const handleQuantityChange = (productId, quantity) => {
    dispatch(
      updateQuantityc({ username: currentUser?.username, productId, quantity })
    ).then((action) => {
  ...
  const handleRemoveFromCart = (productId) => {
    dispatch(
      removeProductc({ username: currentUser?.username, productId })
    ).then((action) => {
  ...
  const handleApplyDiscountCode = () => {
    try {
      ...
      message.success(
        `Discount code applied. Your discount is ${discountValue * 100}%.`
      );
    } catch (error) {
